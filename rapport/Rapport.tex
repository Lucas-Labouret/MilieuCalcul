...
\begin{figure}[H]
	\centering\includegraphics[width=0.9\linewidth]{assets/OR_Reduction_Tables.png}
\end{figure}

Since 0 is the neutral element of the OR operator, we use it to initialize our result table. Grey values in the grid indicate that the cell does correspond to a vertex, which means it contains a garbage value that can be safely ignored as it will never be used.
...
Most obviously, increasing the density lowers the space between the vertices and is therefore correlated to reducing the $\Delta$'s \textit{up to a certain point}. However, increasing the density too much starts to cause vertices to get "too close" to each other, which lowers the performance of the canning as the unnatural positioning of the vertices causes the apparition of new masks.

On the other hand, decreasing the starting density \textit{up to a certain point} often yields better result when the canning is optimized through simulated annealing. But, like before, decreasing the density too much creates a starting canning so bad that it ends up hindering the annealing process.
