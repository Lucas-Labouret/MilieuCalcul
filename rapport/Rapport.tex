...
\titlepic{\includegraphics[width=0.87\linewidth]{assets/Circle500.png}}
...
This report summarizes the work I performed at LISN under the supervision of Frédéric Gruau. It is the continuation of a research project I did under him last year  my Licence degree's curriculum. It is part of a long term research project about the concept of "blob computing"\supercite{blob_computing1}. 

My presence at LISN was spread over two periods~: first, a TER from January to April 2025, then an internship from May to July 2025.
During the TER, I performed research about the simulation of blob computing on classical computers, and wrote a GUI application that allows users to visualize and interact with my results.
The internship then focused on improving and expanding on the results I obtained during the TER.

The program that I produced during this internship can be found \href{https://github.com/Lucas-Labouret/MilieuCalcul}{\textbf{here}}.

$ $\\
\small\textit{Note~: Chapters 1 and 2 are identical the TER report I already submitted. I chose to include them anyway as they are necessary to understand the new chapter 3.}
...
\begin{figure}[h]
	\centering\includegraphics[width=0.6\linewidth]{assets/handdrawn_medium.png}
...
\begin{figure}[h]
	\centering\includegraphics[width=0.5\linewidth]{assets/voronoi_spread.png}
	\caption{
...
\section{Some conventions going forward}
...
- We only work on vertex cannings, and rely entirely on the algorithm described in \ref{vertexCanningCompleter} to convert them into total vertex cannings.

\section{Platform-CA2\supercite{platform_CA2} and Evaluation}

I spent the first two weeks of the internship reading the source code of the simulation platform at \cite{platform_CA2}. I was hoping to be able to implement the simulation of amorphous media into it, although we ultimately decided against it as the internship was simply too short for this to be a realistic goal.

So how does this platform work, and how exactly do cannings help ?

\subsection{How is SIMD applied ?}

To simplify things, let's assume that vertices can communicate between themselves directly
without the need for transfer locus intermediary edges, and that they each contain a boolean value. We will now take the example of an OR reduction on the third line of vertices of a medium $M$. It is a simple operation~: each vertex has (up to) 8 neighbors, and we compute its new value as $Value(neighbor_0)\lor\dots\lor Value(neighbor_7)$.

First, we fill two 2D arrays~: one that will contain the starting values of the reduction, i.e. the values contained in the vertices before the reduction, and the other will be our result table which and will contains the new values of our vertices after the reduction. These arrays are linked to a canning $C$ of $M$ in that the value in the cell $(Y, X)$ correspond to the value of the vertex at position $(Y, X)$ in $C$. 

\begin{figure}[H]
	\centering\includegraphics[width=0.7\linewidth]{assets/OR_Reduction_Tables.png}
\end{figure}
...
\begin{figure}[H]
	\centering\includegraphics[width=0.9\linewidth]{assets/OR_Reduction_Bitshift.png}
\end{figure}
...
\begin{figure}[H]
	\centering\includegraphics[width=0.9\linewidth]{assets/OR_Reduction_Masks.png}
\end{figure}

First, we select all the lines that contain a second neighbor of a vertex on the third line. Note that a line can be selected multiple times if reaching the appropriate cells requires different bitshifts. Then, we apply a mask to each line selected to destroy all the information that we don't need so that it doesn't interfere with the result. Finally, we recombine all these lines by consecutively OR'ing them to to the values currently stored in our result table.\\

Now, in reality, vertices don't communicate directly but the two concepts above apply no matter which types of loci are communicating as they are all stored as n-dimensional arrays with the last dimension being represented as an integer instead of an actual array.

\subsection{Exact Measurement of the Performance of a Canning}
...
\begin{enumerate}[noitemsep, nosep]
	\item The number of lines in the canning
	\item The number of fragments of length $32-2\Delta x$ in each line (i.e. the $\Delta x$)
	\item The number of masks per fragment
\end{enumerate}
Additionally, we also want to minimize the $\Delta y$ as it determines how many lines must remain readily accessible in memory at any given point in time.
...
\subsection{SIMD limitations}
\label{simd_limitations}
...
\subsection{Importance of $\Delta$'s and Density}

As we just saw, the $\Delta x$ is another important factor on the performance of a canning, as is indicates how much information can be stored in a line fragment. The $\Delta y$ is also important, as it determines how many lines need to be loaded in memory at any time. It is sufficient to compute them on the vertex canning alone as it fully determines the entire canning.\\
...
\section{Creating Vertex Cannings From Scratch}

\subsection{Revisiting the Rounded Coordinates Algorithm$^{\ref{partial_canning}}$}
...
One way to remove the arbitrariness is to perform a dichotomic search for the minimum working scaling factor. However, this methods yields terrible results (in fact, often worse than simply setting the increment to one). This is caused by the fact that a scaling factor $x_1$ can work while being smaller than a scaling $x_2$ which doesn't, making the dichotomic search ill-suited for this problem. This method had the additional drawback of trying to reduce increase the density of the canning at all cost, even if it would cause it to hurt the overall performance of the canning.\\
...
However, even with the arbitrariness gone, we run into a second issue : this algorithm performs really well on square media, but struggles with rectangular media. This problem arises because we use the same scaling factor for both the $y$ and $x$ coordinates : since the rectangle is longer than it is tall (or vice-versa), we require a larger horizontal scaling factor (i.e. we want longer lines to fit all our vertices), but because we use the same scaling factor for both coordinates, we also en up with taller, emptier columns. The solution is then easy : using two separated scaling factors.
...
\begin{itemize}[noitemsep, nosep, label=-]
	\item This systematically results in cannings whose number of lines and columns are powers of 2, which may not always be well-suited.
	\item Two vertices could both be placed in the same line and the same column. This means that they would be placed in the same cell, which is disallowed by definition.
	\item This would create lines and columns that are extremely uneven. 
\end{itemize}$ $
...
\begin{enumerate}[noitemsep, nosep]
	\item 
	If there is one column (resp. line) to create
	Put the expected amount of vertices in it, starting from the closest to the center of the current part.
	\item 
	If there is a pair number of columns (resp. lines) to create\\
	...
	\item 
	If there is odd number of columns (resp. lines) to create\\
	Create a column (resp. line) and fill it with the expected amount of the vertices that are closest to the middle of the current part.
	Repeat this algorithm on the the two parts that are left on each side of side of the new column (resp. line). 
\end{enumerate}$ $

This approach solves the problem of having uneven lines and columns and provides number of lines and columns that more fitted to the medium, but doesn't solve the issue of vertices sharing the same cell and even result in vertices that are not placed at all. Placing those missing vertices correctly while separating those which share the same cells is not trivial at all, and ultimately I couldn't find a goo way to do it. 

\section{Optimizing Existing Vertex Cannings}
...
\subsection{Adaptative Grid Method}
...
Disappointingly, these methods do not always allow to reach a maximum delta of 2 on large mediums ($\ge$ 2025 vertices), however I do strongly believe that such a result is always attainable, given how little problematic pairs of vertices there even in the largest grids. In particular, I believe that the Local Resolution methods has a strong potential. 
...
\chapter*{Appendix~: Use the application}
\label{appendix}
\addcontentsline{toc}{chapter}{\nameref{appendix}}
...
Upon launching the application, the user will be faced with the main window. It divided into 5 parts~:

\begin{figure}[H]
	\centering\includegraphics[width=1\linewidth]{assets/GUI_Main.png}
	\caption{}
	\label{fig:gui_main}
\end{figure}

\begin{enumerate}
	\item \textbf{Medium shapes}\\
	This bar allows to chose the medium shape to use.
	
	\item \textbf{Medium generation and optimization}\\
	This bar contains~:
	\begin{enumerate}[noitemsep, nosep]
		\item A series of user input fields that is defined separately for each medium shape.
		\item A "Generate" button which creates a new random medium using the user-provided parameters.
		\item A "Triangulate" button which applies a Delaunay triangulation of the medium.
		\item An "FPO" button which both triangulates the medium and apply the FPO algorithm using the parameter provided the parameter panel.
		\item A "Masks" button, which opens a new window displaying information about the masks needed to perform a communication using the current canning. 
	\end{enumerate}

	\item \textbf{Main view}\\
	This part displays the currently shown medium and canning.
	
	\item \textbf{Parameter panel}\\
	...
	The FPO category changes how the the FPO should be applied. The "Set iteration" mode performed as many iteration of the algorithm as indicated in the textbox directly underneath it. The "To convergence" mode keep going until the medium is "sufficiently" optimized (as defined in the textbox directly underneath it, the closer to 1 the better), or until no further progress is made. \\
	\warning The "To convergence" mode is bugged. Using 7 iterations should be enough for virtually any medium.
	
	\item \textbf{Information bar}\\
	This bar displays various information about the currently shown medium an canning.
	
	\item \textbf{Save management}\\
...
\section{How to add new medium shapes}

Start by creating a new class extending either \verb|SoftBorderedMedium| or \verb|HardBorderedMedium| (or \verb|Medium| directly). In the case of a \verb|HardBorderedMedium| extension, it is recommanded to override the \verb|delaunayTriangulate| method to repair any incorrect link between vertices on the border that may occur after triangulation.
...
Create a new class extending \verb|SavefileManager|. It should only override the \verb|makeMedium| method with \verb|public <MediumClass> makeMedium() { return new <MediumClass>(); }|\\

Create a class extending \verb|MediumApp|. This class should define a constructor creating all the input you may need to add to the top tool bar (2 in figure \ref{fig:gui_main}), and create an instance of the suitable \verb|SavefileManager| subclass. This class should also override the \verb|generate| method to create the new medium according to user specification.

\section{How to add new cannings}

\textbf{To create a standalone canning}~:\\
Simply create a class implementing the \verb|VertexCanning|. Its instances can later be passed to a \verb|VertexCanningCompleter| to convert them to total cannings.

While it is possible to create a class directly implementing the \verb|Canning| interface, this approach is not recommended, as the backup of such cannings is currently not supported.\\
$ $\\
\textbf{To create an annealing-based canning}~:\\
Create a new class extending \verb|Annealer<VertexCanning>|, and pass its instances to a \verb|VertexCanningAnnealer| along with a \verb|VertexCanning| to use as a seed for the annealing process. Finally, give the instance of the \verb|VertexCanningAnnealer| to a \verb|VertexCanningCompleter|.\\
$ $\\
\textbf{To add a canning to the GUI}~:\\
In the source code of the class \verb|MediumApp|, after the line \verb|ToggleGroup canGroup = new ToggleGroup();| in \verb|fillSidePanel()|~:
\begin{enumerate}[noitemsep, nosep]
	\item Create a new \verb|RadioButton rd| and add it to the toggle group with \verb|rd.setToggleGroup(canGroup)|.
	
	\item In the lambda passed to \verb|canGroup.selectedToggleProperty().addListener()|, add 
	\begin{verbatim}
		else if (newVal == rd) { canningFactory = m -> new NewCanning(parameters...) }
	\end{verbatim}
	
	\item Add \verb|rd| to the constructor of \verb|VBox canning|.
\end{enumerate}
