...
class TriangleSoup {
    ...
    /**
     * For all triangles pair (t1,t2) tests if they are neigbors, and if so, position ta, tb or tc.
     * The complexity is quadratic, that is not very good.
     */
    public void computeNeighbor() {

        for (int i = 0; i < triangleSoup.size(); i++) {
            Triangle2D triangle1 = triangleSoup.get(i);
            for (Triangle2D triangle2 : triangleSoup)
                triangle1.computeNeighbour(triangle2);
        }
