...
public class DivideAndConquerVCanning implements VertexCanning {
    ...
    private void applyTargetDensity(boolean lines, HashMap<VertexCoord, Vertex> reversedCanning) {
        ...
        for (int i = (lines? height : width) - 1; i >= 0; i--) for (int j = 0; j < (lines ? width : height); j++) {
            ...
            if (relativePosition < Math.sqrt(targetDensity)) continue;
            ...
            Vertex nextVertex;
            for (int k = i; k < (lines ? height : width); k++) {
                ...
                if (currentVertex != null) {
                    ...
                    if ( lines && nextCoord.Y() >= height) height = nextCoord.Y()+1;
                    if (!lines && nextCoord.X() >= width ) width  = nextCoord.X()+1;
                }
    ...
    private void placeMissingVertices(
        ...
        for (int y = 0; y < height; y++) for (int x = 0; x < width; x++) {
            ...
            if (reversedCanning.get(coord) == null) holes.add(coord);
        }
        ...
        for (Vertex vertex : missingVertices) {
            ...
            if (lowBound < 0) {
                lowBound = 0;
                while (!lineMap.containsKey(ySorted.get(lowBound))) lowBound++;
            }
            else while (lowBound > 0 && !lineMap.containsKey(ySorted.get(lowBound))) lowBound--;

            if (highBound >= ySorted.size()) {
                highBound = ySorted.size() - 1;
                while (!lineMap.containsKey(ySorted.get(highBound))) highBound--;
            }
            else while (highBound < ySorted.size() - 2 && !lineMap.containsKey(ySorted.get(highBound))) highBound++;

            if (leftBound < 0) {
                leftBound = 0;
                while (!colMap.containsKey(ySorted.get(leftBound))) leftBound++;
            }
            else while (leftBound > 0 && !colMap.containsKey(xSorted.get(leftBound))) leftBound--;

            if (rightBound >= xSorted.size()) {
                rightBound = xSorted.size() - 1;
                while (!colMap.containsKey(ySorted.get(rightBound))) rightBound--;
            }
            else while (rightBound < xSorted.size() - 2 && !colMap.containsKey(xSorted.get(rightBound))) rightBound++;
            ...
            for (VertexCoord hole : holes) {
                double dist = Math.abs(hole.X() - coord.X()) + Math.abs(hole.Y() - coord.Y());
